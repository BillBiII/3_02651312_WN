...
def schrodinger_exact(t, ψ):
    ...
    """
    ψ = ψ.astype(complex)
    return -1j * hamil_exact(t, ε, γ, ω) @ ψ
...
def inner_product(f, g, t,  frame_transform_f=False, frame_transform_g=False):
    """
    ...
    for i in range(f.shape[1]):
        ...
        if frame_transform_f is True:
            rot_adj_f = rotating_frame_adjustment(t[i])
            ψ_f = rot_adj_f @ ψ_f
        if frame_transform_g is True:
            rot_adj_g = rotating_frame_adjustment(t[i])
            ψ_g = rot_adj_g @ ψ_g
    ...
    average_fidelity = trapezoid(fidelity, t) / (t[-1] - t[0]) # This computes an average
...
print(inner_product(schrodinger_exact_solution.y,schrodinger_RWA_solution.y, t_axis, True))
# We've now REASONABLY confirmed that the Exact Hamiltonian and RWA Hamiltonian, under a frame rotating at ω, are very similar to eachother graphically. This means we can use the RWA estimate.
...
# Quantum measurements are commented out since you cannot obtain state after the measurements are made. Instead, you get one of the basis states. (This might need a bit more explaining).
